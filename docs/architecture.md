...
**Generation contract.** The model must return `AnswerDraft` via `with_structured_output(..., method="json_schema", strict=True, include_raw=True)`: status (`answered` / `partially_answered` / `insufficient_evidence`), answer, claims with `chunk_id` + verbatim `quote`, `missing_information`, and `premise_issue` ([schemas.py](../src/website_rag/schemas.py)). The versioned prompt ([prompts/answer_v1.md](../src/website_rag/prompts/answer_v1.md)) treats evidence as untrusted data, forbids outside knowledge, and asks for explicit correction of false premises. Evidence is serialized in `<chunk id=...>` blocks and any `<chunk`/`</chunk` inside page text is escaped.

**Citation checks** ([citations.py](../src/website_rag/citations.py)): cited ID must be in the supplied context; the quote must appear in that chunk after normalization (NFKC, typographic quotes and dashes folded, backticks removed, whitespace collapsed, case-folded; `...` joins in-order excerpts; minimum 8 characters). A claim with any invalid evidence is rejected. If all claims pass, the answer is shown; if some fail, the free-text answer is withheld and only verified claims are shown; if none pass, the result is a `citation_invalid` error. Source URLs, titles, and section labels are joined from index metadata, never taken from model output.
